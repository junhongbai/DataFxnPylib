...
def structures_to_column(structures: List[Optional[str]], column_name: str = 'Compressed PDB Structures') \
        -> Tuple[ColumnData, List[Optional[Notification]]]:
    """
    Converts a list of PDB structure files as strings into a Spotfire ColumnData object

    :param structures:  list of Bio.PDB.Structure objects
    :param column_name:  a string with the desired column name (the default is "Compressed PDB Structures"

    :return: Tuple of ColumnData object and List of Notification objects
    """
    ...
    try:
        for index, pdb_data in enumerate(structures):
            if pdb_data:
                pdb_zip = gzip.compress(pdb_data.encode())
                pdb_enc = base64.b64encode(pdb_zip).decode('utf8')
                encoded_structures.append(pdb_enc)
            else:
                encoded_structures.append(None)
    except Exception as ex:
        encoded_structures.append(None)
        notifications.append(Notification(level = NotificationLevel.WARNING,
                                          title = 'structures_to_column',
                                          summary = f'Error for structure {index}./n{ex.__class__} - {ex}',
                                          details = f'An error occurred during PDB data compression.\n' +
                                                    f'{traceback.format_exc()}'))

    column = ColumnData(name = 'Compressed PDB Structures', dataType = DataType.BINARY,
                        contentType = 'chemical/x-pdb', values = encoded_structures,
                        properties = {'Dimension': '3', 'Alias': column_name})

    return column, notifications
